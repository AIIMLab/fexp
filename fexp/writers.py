...
def write_image(data, filepath, compression=True, metadata=None, resample=False):
    ...
    if resample and metadata["orig_spacing"] != metadata["spacing"]:
        sitk_image, _ = resample_sitk_image(
            sitk_image, spacing=metadata["orig_spacing"], interpolator=resample
        )
    ...
    if any([str(filepath).endswith(ext) for ext in possible_exts]):
        try:
            sitk.WriteImage(
                sitk_image,
                str(filepath),
                True if str(filepath).endswith(".nii.gz") else compression,
            )
        ...
        if not data.ndim == 3:
            raise ValueError(
                f"For dicom series, only 3D data is supported. Got {data.ndim}."
            )

        series_in_directory = [
            int(_.split("-")[1]) for _ in glob(os.path.join(filepath, "IMG-*.dcm"))
        ]
        ...
        series_tag_values = [
            ...
            (
                "0008|103e",
                metadata.get(
                    "series_description", f"fexp generated number: {curr_series}"
                ),
            ),  # Series Description
        ]
        ...
        for idx in range(sitk_image.GetDepth()):
            ...
            image_slice.SetMetaData(
                "0020|0032",
                "\\".join(
                    map(str, sitk_image.TransformIndexToPhysicalPoint((0, 0, idx)))
                ),
        ...
        raise ValueError(
            f"Filename extension has to be one of {possible_exts} or a directory."
        )
