...
def read_image(filename, dtype=None, no_metadata=False, force_2d=False, **kwargs):
    ...
    if filename.suffix.lower() == ".dcm" and kwargs.get("dicom_keys", None):
        ...
        for v in kwargs["dicom_keys"]:
            dicom_data[v] = (
                None if v not in metadata_keys else sitk_image.GetMetaData(v).strip()
            )
...
def resample_sitk_image(sitk_image, spacing=None, interpolator=None, fill_value=0):
    ...
    if not spacing:
        ...
        new_spacing = [
            float(s) if s else orig_spacing[idx] for idx, s in enumerate(spacing)
        ]
    ...
    new_size = [
        int(s) if spacing[idx] else int(orig_size[idx])
        for idx, s in enumerate(new_size)
    ]
