...
# Elementary subroutines for computing 2d separable convolution with various methods.  Not all sub routines are fully
# fully optimized computational speed can be gained at various places. Module is still under development.
...
def mult2ind(mult_ind, dim_size):
    """
    Convert d-dimensional multi-index to linear index with respect to given array size using row-major ordering
...
def complex_mult(u, v):
    ...
    u: float-valued Pytorch tensor of size [ .... 2]
        array of arbitrary size, where the last dimension corresponds to the real and imaginary components
    v: float-valued Pytorch tensor of size [ .... 2]
        array of same size as u, where the last dimension corresponds to the real and imaginary components
...
def two_sided_order(a):
    """
    Determine order n associated to sequence so that a = [a_{1-n} ... a_{0} ... a_{n}] if n is even and
    a = [a_{1-n} ... a_{0} ... a_{n-1]}] if n is odd.
...
def toeplitz_matrix(a, num_shifts):
    """
    Construct Toeplitz matrix. Currently only implemented for odd sequence a with odd length
...
def conv1d(a, b, mode="trunc", method="toeplitz"):
    """
    Compute 1d two-sided convolution a * b. Coefficients are interpreted as [a_{:, 1-n} ... a_{0} ... a_{:, n-1}] if n
    is odd and [a_{:, 1-n} ... a_{:, 0} ... a_{:,n}] if n is even (the same holds for b). If the chosen method is FFT,
    then a_{i,:} * b_{i, :} is computed for 0 <= i <= batch_size - 1. If the chosen method if Toeplitz, then b should
    only have one row; in this case a_{i, :} * b is computed for 0 <= i <= batch_size - 1. Currently, only the truncated
    convolution is implemented for the Toeplitz-approach.
    ...
    mode: str in {full, trunc}, optional
        compute all convolution terms or truncate to nonzero-coefficients determined by (maximal) length of input
...
def conv2d_separable(a, w, mode="trunc", method="toeplitz"):
    """
    Compute 2d two-sided convolution a * w, where a is an of array coefficients and w1, w2 define a "separable" array w
    Todo: code is easily generalized to arbitray dimensions
    ...
    mode: str in {full, trunc}, optional
        compute all convolution terms (full) or truncate (trunc) to nonzero-coefficients determined by (maximal)
        length of input
